...
private:
	...
	double ugol(float x1f1, float y1f1, float x2f1, float y2f1, float x1f2, float y1f2, float x2f2, float y2f2)
	{
		float x1f = x1f1 - x2f1;
		float y1f = y1f1 - y2f1;
		float x2f = x1f2 - x2f2;
...
public:
	figure(float X1, float X2, float X3, float X4, float Y1, float Y2, float Y3, float Y4)
	{
		...
		S = 0.5 * sin(ugol(X1, Y1, X3, Y3, X2, Y2, X4, Y4) / (180 / PI)) * dist(X1, Y1, X3, Y3) * dist(X2, Y2, X4, Y4);
